  ...
  const addFiles = useCallback((incoming: File[]) => {
    ...
    const valid = incoming.filter((f) => {
      if (!f.name.toLowerCase().endsWith(".pdf")) {
        clientErrors.push(`${f.name}: only PDF files are accepted here.`);
        return false;
      }
    ...
    setFiles((prev) => {
      ...
      if (combined.length > MAX_FILES) { clientErrors.push(`Maximum ${MAX_FILES} PDFs.`); return prev; }
    ...
    >
      ...
      {!result ? (
        <>
          <div className={s.card}>
            <div className={t.limitsRow}>
              <span className={t.limitChip}>PDF only</span>
            ...
            >
              ...
              <span className={t.dropZoneTitle}>{dragging ? "Drop PDFs here" : "Drag & drop PDFs here"}</span>
              <span className={t.dropZoneSub}>PDF files only or click to browse</span>
              <span className={t.browseBtn} onClick={(e) => { e.stopPropagation(); fileInputRef.current?.click(); }}>Browse PDFs</span>
            </div>
            <input ref={fileInputRef} type="file" multiple accept="application/pdf" hidden onChange={onFileChange} />
          </div>

          {files.length > 0 && (
            <div className={s.card}>
              <div className={s.rowBetween}>
                <span className={s.label}>{files.length} PDF{files.length !== 1 ? "s" : ""} · {formatBytes(totalSize)}</span>
        ...
        <div className={t.successBox} ref={resultRef}>
          <span className={t.successTitle}>{expired ? "Link Expired" : "PDFs Ready to Share"}</span>
          ...
          <button className={t.shareAgainBtn} onClick={reset}>Share more PDFs</button>
        </div>
      )}
    </ToolPageShell>
  );
